 ...
 */
public class SilentInstallStaging extends AlertActivity {
    private static final String LOG_TAG = SilentInstallStaging.class.getSimpleName();
    ...
    private final class StagingAsyncTask extends AsyncTask<Uri, Void, Boolean> {
        ...
        protected void onPostExecute(Boolean success) {
            if (success) {
                ...
                boolean isUpdate = checkIsUpdate(installIntent);
                if(isUpdate){
                    installIntent.setClass(SilentInstallStaging.this, SilentDeleteStagedFileOnResult.class);
                    installIntent.setFlags(Intent.FLAG_ACTIVITY_LAUNCH_ADJACENT|Intent.FLAG_ACTIVITY_NEW_TASK);
                } else {
                    installIntent.setClass(SilentInstallStaging.this, DeleteStagedFileOnResult.class);
                    installIntent.addFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION);
                }
                startActivity(installIntent);

                SilentInstallStaging.this.finish();
            } else {
                showError();
            }
        }
    ...
    private boolean checkIsUpdate(Intent intent) {
        final Uri packageUri  = intent.getData();
        File sourceFile = new File(packageUri.getPath());
        PackageInfo mPkgInfo = PackageUtil.getPackageInfo(this, sourceFile,
                PackageManager.GET_PERMISSIONS);
        PackageUtil.AppSnippet mAppSnippet = PackageUtil.getAppSnippet(this, mPkgInfo.applicationInfo, sourceFile);
        ...
        if (mAppSnippet != null) {
            ...
            try {
                ApplicationInfo mAppInfo = mPm.getApplicationInfo(pkgName,
